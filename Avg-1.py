# -*- coding: utf-8 -*-
"""
Created on Tue Aug 29 16:09:31 2023
...
df = pd.read_excel(r"C:\Users\Esra Simsek\Desktop\Community Discharge\Community discharge-July 2023\Community-discharge-sitrep-monthly-data-webfile-July2023.xlsx", sheet_name='Table 2')
...
grouped['performance_group'] = pd.qcut(grouped['value'], q=3, labels=['The best performers', 'The medium performers', 'The worst performers'])

# Count the occurrences of each performance group
performance_counts = grouped['performance_group'].value_counts()

# Determine performance groups
best_performers = performance_counts.idxmax()
medium_performers = performance_counts.idxmin()
worst_performers = performance_counts.drop([best_performers, medium_performers]).index[0]

# Filter performers based on performance groups
best_performers_list = grouped[grouped['performance_group'] == best_performers]['Name'].tolist()
medium_performers_list = grouped[grouped['performance_group'] == medium_performers]['Name'].tolist()
worst_performers_list = grouped[grouped['performance_group'] == worst_performers]['Name'].tolist()


#grouped.to_csv("8-avg-Table2.csv", index=False)

#print("Best performers:", best_performers_list)
#print("Medium performers:", medium_performers_list)
#print("Worst performers:", worst_performers_list)

# Ensure all lists have the same length
max_length = max(len(best_performers_list), len(medium_performers_list), len(worst_performers_list))
best_performers_list.extend([''] * (max_length - len(best_performers_list)))
medium_performers_list.extend([''] * (max_length - len(medium_performers_list)))
...
performers_df = pd.DataFrame({
    ...
    'Medium Performers': medium_performers_list,
    'Worst Performers': worst_performers_list
})

performers_df.to_csv('performers2.csv', index=False)
